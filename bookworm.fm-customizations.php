...
function bookworm_post_class_meta_box($object, $box)
{
  ...
  <p>
    <label for="bookworm-mike-rating"><?php _e(
      "Mike's Rating",
      "example"
    ); ?></label>
    <br />
    <input class="widefat" type="text" name="bookworm-mike-rating" id="bookworm-mike-rating" value="<?php echo esc_attr(
      get_post_meta($object->ID, "bookworm_mike_rating", true)
    ); ?>" size="30" />
    <label for="bookworm-joe-rating"><?php _e(
      "Joe's Rating",
      "example"
    ); ?></label>
    <br />
    <input class="widefat" type="text" name="bookworm-joe-rating" id="bookworm-joe-rating" value="<?php echo esc_attr(
      get_post_meta($object->ID, "bookworm_joe_rating", true)
    ); ?>" size="30" />
    <label for="bookworm-pro-episode"><?php _e(
      "Pro Episode URL",
      "example"
    ); ?></label>
    <br />
    <input class="widefat" type="text" name="bookworm-pro-episode" id="bookworm-pro-episode" value="<?php echo esc_attr(
      get_post_meta($object->ID, "bookworm_pro_episode", true)
    ); ?>" size="30" />
    <label for="bookworm-bootleg-episode"><?php _e(
      "Bootleg Episode URL",
      "example"
    ); ?></label>
    <br />
    <input class="widefat" type="text" name="bookworm-bootleg-episode" id="bookworm-bootleg-episode" value="<?php echo esc_attr(
      get_post_meta($object->ID, "bookworm_bootleg_episode", true)
    ); ?>" size="30" />
...
echo $bookID;
?>" <?php if ($bookID == $bookworm_book_id) {
  echo "selected='selected'";
} ?>><?php echo $bookTitle; ?></option>
      <?php
      endforeach; ?>
    </select>
  </p>
...
function bookworm_book_status_meta_box($object, $box)
{
  ...
  <?php wp_nonce_field(basename(__FILE__), "bookworm_nonce"); ?>
  <label for="bookworm-amazon-link"><?php _e(
    "Amazon Link",
    "example"
  ); ?></label>
    <br />
    <input class="widefat" type="text" name="bookworm-amazon-link" id="bookworm-amazon-link" value="<?php echo esc_attr(
      get_post_meta($object->ID, "bookworm_amazon_link", true)
    ); ?>" size="30" />
  <p>
    <select class="widefat" name="bookworm-book-status" id="bookworm-book-status"><?php
    $bookworm_book_status = get_post_meta(
      $object->ID,
      "bookworm_book_status",
      true
    );
...
function bookworm_save_post_class_meta()
{
  ...
  if (
    !isset($_POST["bookworm_nonce"]) ||
    !wp_verify_nonce($_POST["bookworm_nonce"], basename(__FILE__))
  ) {
    return;
  }
  ...
  $new_mike_rating_value = isset($_POST["bookworm-mike-rating"])
    ? $_POST["bookworm-mike-rating"]
    : "";
  $new_joe_rating_value = isset($_POST["bookworm-joe-rating"])
    ? $_POST["bookworm-joe-rating"]
    : "";
  $new_amazon_link_value = isset($_POST["bookworm-amazon-link"])
    ? $_POST["bookworm-amazon-link"]
    : "";
  $new_book_id_value = isset($_POST["bookworm-book-id"])
    ? $_POST["bookworm-book-id"]
    : "";
  $new_book_status_value = isset($_POST["bookworm-book-status"])
    ? $_POST["bookworm-book-status"]
    : "";
  $pro_episode_value = isset($_POST["bookworm-pro-episode"])
    ? $_POST["bookworm-pro-episode"]
    : "";
  $bootleg_episode_value = isset($_POST["bookworm-bootleg-episode"])
    ? $_POST["bookworm-bootleg-episode"]
    : "";

  update_bookworm_meta(
    $post->ID,
    "bookworm_mike_rating",
    $new_mike_rating_value
  );
  update_bookworm_meta($post->ID, "bookworm_joe_rating", $new_joe_rating_value);
  update_bookworm_meta(
    $post->ID,
    "bookworm_amazon_link",
    $new_amazon_link_value
  );
  update_bookworm_meta($post->ID, "bookworm_book_id", $new_book_id_value);
  update_bookworm_meta(
    $post->ID,
    "bookworm_book_status",
    $new_book_status_value
  );
  update_bookworm_meta($post->ID, "bookworm_pro_episode", $pro_episode_value);
  update_bookworm_meta(
    $post->ID,
    "bookworm_bootleg_episode",
    $bootleg_episode_value
  );
}
...
function booklist_func()
{
  global $wpdb;
  $booklist = "";
  $args = ["post_type" => "post", "posts_per_page" => 5000];
...
function booklist_planned_func()
{
  global $wpdb;
  $booklist = "";
  $args = ["post_type" => "book", "posts_per_page" => 5000];
  $loop = new WP_Query($args);
  while ($loop->have_posts()):
    $loop->the_post();
    $id = get_the_ID();
